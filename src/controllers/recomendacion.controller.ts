...
const includeSolicitud: Prisma.solicitudes_recomendacionInclude = {
...
async function generarRecomendacionesAuto(opts: {
  tipo_interes_ids: number[];
  presupuesto: number | undefined;
  cantidad_personas: number | undefined;
}): Promise<{ sucursal_id: number; puntaje: number }[]> {
  ...
  const esTuristico = (categoria: { nombre: string; permite_reservas: boolean | null } | null) =>
    categoria !== null && (categoria.permite_reservas === true || /turistico|tour/i.test(categoria.nombre));
  ...
  const conPuntaje = sucursales.map((s) => {
    ...
    // Categoría turística
    if (esTuristico(s.empresas?.categorias_negocio ?? null)) score += 10;
    ...
    if (presupuesto !== undefined && costeEntrada > 0) {
      if (costeEntrada * personas <= presupuesto) score += 12;
      else score -= 8;
    ...
    if (s.empresas?.verificado === true) score += 4;
    if (s.empresas?.destacado === true) score += 2;
...
export const crearSolicitud = async (req: Request, res: Response) => {
  try {
    ...
    const ids = Array.isArray(tipo_interes_ids)
      ? [...new Set(tipo_interes_ids.map((v: unknown) => Number(v)).filter((n: number) => Number.isInteger(n) && n > 0))]
      : [];
    if (ids.length > 0) {
...
export const recomendar = async (req: Request, res: Response) => {
  try {
