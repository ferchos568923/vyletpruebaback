...
export const agregar = async (req: Request, res: Response) => {
  try {
    ...
    if (existe) return res.status(400).json({ error: 'Este usuario ya es empleado de esta empresa' });
...
export const listarSucursalesEmpleado = async (req: Request, res: Response) => {
  try {
...
export const asignarSucursalesEmpleado = async (req: Request, res: Response) => {
  try {
    const ctx = await verificarEmpleado(req, res);
    if (!ctx) return;
    ...
    const ids = [...new Set(sucursal_ids.map((v: unknown) => Number(v)).filter((n: number) => Number.isInteger(n) && n > 0))];
    ...
    await prisma.$transaction([
      ...
      ...ids.map((sucursal_id) =>
        prisma.empleado_sucursales.create({ data: { empleado_id: ctx.empleadoId, sucursal_id } })
      )
    ]);
...
const verificarEmpleado = async (req: Request, res: Response) => {
  const empresaId = Number(req.params.id);
  const empleadoId = Number(req.params.empleadoId);
  const empresa = await prisma.empresas.findUnique({ where: { id: empresaId } });
  if (!empresa) { res.status(404).json({ error: 'Empresa no encontrada' }); return null; }
  if (!(await canGestionarEmpleados(req, empresaId))) {
    res.status(403).json({ error: 'No puedes gestionar empleados de esta empresa' }); return null;
  }
  const empleado = await prisma.empresa_empleados.findFirst({
    where: { id: empleadoId, empresa_id: empresaId }
  });
...
export const listarPermisosEmpleado = async (req: Request, res: Response) => {
  try {
    const ctx = await verificarEmpleado(req, res);
    if (!ctx) return;
...
export const asignarPermisosEmpleado = async (req: Request, res: Response) => {
  try {
    const ctx = await verificarEmpleado(req, res);
    if (!ctx) return;
    ...
    await prisma.$transaction([
      ...
      ...permiso_ids.map((pid) =>
        prisma.empleado_permisos.create({
