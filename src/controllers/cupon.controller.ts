...
const parseNum = (v: unknown): number | null | undefined => {
...
const buildData = (body: any) => {
  ...
  if (body.valor_descuento !== undefined) data.valor_descuento = Number(body.valor_descuento);
  if (body.monto_minimo !== undefined) data.monto_minimo = parseNum(body.monto_minimo);
  if (body.fecha_inicio !== undefined) data.fecha_inicio = body.fecha_inicio === '' ? null : parseFecha(body.fecha_inicio);
  if (body.fecha_fin !== undefined) data.fecha_fin = body.fecha_fin === '' ? null : parseFecha(body.fecha_fin);
  if (body.cantidad_usos !== undefined) data.cantidad_usos = parseNum(body.cantidad_usos);
  return data;
};
...
export const listPublic = async (req: Request, res: Response) => {
  try {
    ...
    const vigentes = cupones.filter((c) => {
      const okFecha =
        (!c.fecha_inicio || new Date(c.fecha_inicio) <= hoy) &&
...
export const adminCanjes = async (req: Request, res: Response) => {
  try {
    const sucursal = await verificarSucursal(req, res);
    if (!sucursal) return;
...
export const canjear = async (req: Request, res: Response) => {
  try {
    const cuponId = Number(req.params.id);
    const cupon = await prisma.cupones.findUnique({ where: { id: cuponId } });
    if (!cupon || !cupon.activo) return res.status(404).json({ error: 'Cupón no encontrado' });
    ...
    if (cupon.sucursal_id) {
      ...
      if (!sucursalCupon || !sucursalCupon.activo) return res.status(404).json({ error: 'Sucursal no encontrada' });
...
export const misCupones = async (req: Request, res: Response) => {
  try {
