...
} satisfies Prisma.plan_viaje_sucursalesInclude;
...
} satisfies Prisma.planes_viajeInclude;

// Precio estimado de un evento dentro del plan (por persona aprox.)
function precioEstimadoEvento(evento: { precio_desde: unknown }): number {
...
async function planConPrecios<
  T extends {
    usuario_id: number;
    sucursales: { sucursales: { id: number; gratuito: boolean | null; precio_adultos: unknown; precio_ninos: unknown } }[];
    eventos?: { eventos: { precio_desde: unknown } }[];
  }
>(plan: T, esMio?: boolean) {
  const sucursales = await Promise.all(
    (plan.sucursales ?? []).map(async (ps) => ({
      ...ps,
  ...
  const total =
    [...sucursales.map((s) => s.precio_estimado ?? 0), ...eventos.map((e) => e.precio_estimado)].reduce(
      (a, b) => a + b,
      0
    );
  return {
...
async function precioEstimadoSucursal(sucursal: { id: number; gratuito: boolean | null; precio_adultos: unknown; precio_ninos: unknown }): Promise<number> {
  if (sucursal.gratuito === true) return 0;
  ...
  if (hab?.precio != null) return Number(hab.precio);
...
export const listarTipos = async (_req: Request, res: Response) => {
  res.json(TIPOS_VIAJE);
};

// GET /api/planes-viaje/sucursales?q=  (público: lista sucursales activas para armar planes)
export const listarSucursalesDisponibles = async (req: Request, res: Response) => {
  try {
...
export const crear = async (req: Request, res: Response) => {
  try {
    ...
    if (ciudad_id !== undefined && ciudad_id !== null && ciudad_id !== '') {
      ...
      if (!ciudad) return res.status(400).json({ error: 'La ciudad seleccionada no existe' });
    }
...
export const listarMios = async (req: Request, res: Response) => {
  try {
    ...
    const conPrecio = await Promise.all(
      planes.map((p) => planConPrecios(p, p.usuario_id === req.user.id))
    );
    res.json(conPrecio);
  } catch (error) {
...
export const obtener = async (req: Request, res: Response) => {
  try {
    ...
    if (!plan) return res.status(404).json({ error: 'Plan de viaje no encontrado' });
...
export const actualizar = async (req: Request, res: Response) => {
  try {
    ...
    if (!existe) return res.status(404).json({ error: 'Plan de viaje no encontrado' });
    ...
    let ciudadId: number | null | undefined;
...
export const agregarSucursal = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    if (!sucursal) return res.status(404).json({ error: 'Sucursal no encontrada o inactiva' });
    ...
    const detalles = await Promise.all(
      (actualizado?.sucursales ?? []).map(async (ps) => ({
        ...ps,
        precio_estimado: await precioEstimadoSucursal(ps.sucursales)
      }))
    );
...
export const quitarSucursal = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
...
export const cambiarPublico = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
...
export const listarPublicos = async (req: Request, res: Response) => {
  try {
...
export const obtenerPublico = async (req: Request, res: Response) => {
  try {
    ...
    if (!plan) return res.status(404).json({ error: 'Plan público no encontrado' });
...
export const copiarPublico = async (req: Request, res: Response) => {
  try {
    const codigo = String(req.params.codigo || '').toUpperCase();
    const plan = await prisma.planes_viaje.findFirst({
      where: { codigo, publico: true },
...
export const buscarAmigos = async (req: Request, res: Response) => {
  try {
...
export const agregarAmigo = async (req: Request, res: Response) => {
  try {
    ...
    if (existe) return res.status(409).json({ error: 'Ese usuario ya está en el plan' });
...
export const quitarAmigo = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
...
export const listarEventosDisponibles = async (req: Request, res: Response) => {
  try {
    const q = String(req.query.q ?? '').trim();
    const where: Prisma.eventosWhereInput = { activo: true };
    if (q) {
      where.OR = [
        { nombre: { contains: q, mode: 'insensitive' } },
...
export const agregarEvento = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    if (existe) return res.status(409).json({ error: 'Ese evento ya está en el plan' });
...
export const quitarEvento = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
