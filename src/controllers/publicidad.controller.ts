...
const buildData = (body: any) => {
  ...
  if (body.presupuesto !== undefined) data.presupuesto = body.presupuesto === '' || body.presupuesto === null ? null : parseDecimal(body.presupuesto);
...
export const adminCreate = async (req: Request, res: Response) => {
  try {
    ...
    if (!data.titulo) return res.status(400).json({ error: 'titulo es requerido' });
    if (!data.empresa_id) return res.status(400).json({ error: 'empresa_id es requerido' });
...
export const adminUpdate = async (req: Request, res: Response) => {
  try {
    ...
    if (!publicidad) return res.status(404).json({ error: 'Publicidad no encontrada' });
