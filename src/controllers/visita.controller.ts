...
const serializar = (r: any) => ({ ...r, hora_visita: horaStr(r.hora_visita) });
...
export const disponibilidad = async (req: Request, res: Response) => {
  try {
    ...
    if (fecha && isNaN(fecha.getTime())) return res.status(400).json({ error: 'fecha inválida' });

    const where: any = { sucursal_id: sucursalId, estado: { in: estadosOcupan } };
    if (fecha) where.fecha_visita = fecha;
...
export const crearReservaVisita = async (req: Request, res: Response) => {
  try {
    const sucursalId = Number(req.params.id);
    const sucursal = await prisma.sucursales.findFirst({ where: { id: sucursalId, activo: true } });
    if (!sucursal) return res.status(404).json({ error: 'Sucursal no encontrada' });
    ...
    if (isNaN(fecha.getTime())) return res.status(400).json({ error: 'fecha_visita inválida' });
...
export const listarReservasVisita = async (req: Request, res: Response) => {
  try {
    const sucursalId = Number(req.params.id);
...
export const actualizarEstadoVisita = async (req: Request, res: Response) => {
  try {
    const sucursalId = Number(req.params.id);
...
export const misReservasVisita = async (req: Request, res: Response) => {
  try {
    const reservas = await prisma.reservas_visita.findMany({
...
export const registrarVisita = async (req: Request, res: Response) => {
  try {
    const sucursalId = Number(req.params.id);
    if (!sucursalId) return res.status(400).json({ error: 'ID inválido' });
