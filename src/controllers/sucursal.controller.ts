...
export const listarTuristicas = async (req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    if (req.user) {
      const conGestion = await Promise.all(
        sucursalesConReservas.map(async (s) => ({ ...s, puedoGestionar: await canGestionarSucursal(req, s) }))
      );
...
export const getById = async (req: Request, res: Response) => {
  try {
    ...
    const resultado: any = { ...sucursal, reservas_habilitadas: reservasHabilitadas };
    if (req.user) {
...
export const update = async (req: Request, res: Response) => {
  try {
    const sucursal = (req as any).sucursal ?? (await prisma.sucursales.findUnique({ where: { id: Number(req.params.id) } }));
    if (!sucursal) return res.status(404).json({ error: 'Sucursal no encontrada' });
    ...
    if (req.body.latitud !== undefined) data.latitud = Number(req.body.latitud);
    if (req.body.longitud !== undefined) data.longitud = Number(req.body.longitud);
    if (req.body.precio_ninos !== undefined) data.precio_ninos = req.body.precio_ninos === '' ? null : Number(req.body.precio_ninos);
    if (req.body.precio_adultos !== undefined) data.precio_adultos = req.body.precio_adultos === '' ? null : Number(req.body.precio_adultos);
    if (req.body.aforo_maximo !== undefined) data.aforo_maximo = req.body.aforo_maximo === '' ? null : Number(req.body.aforo_maximo);
    if (req.body.gratuito !== undefined) data.gratuito = Boolean(req.body.gratuito);
...
export const setServicios = async (req: Request, res: Response) => {
  try {
    const sucursal = (req as any).sucursal ?? (await prisma.sucursales.findUnique({ where: { id: Number(req.params.id) } }));
    if (!sucursal) return res.status(404).json({ error: 'Sucursal no encontrada' });
    ...
    const ids = Array.isArray(servicio_ids) ? [...new Set(servicio_ids.map((v: unknown) => Number(v)).filter((n: number) => Number.isInteger(n) && n > 0))] : [];
    if (ids.length > 0) {
    ...
    await prisma.$transaction([
      ...
      prisma.sucursal_servicios.createMany({ data: ids.map((servicio_id: number) => ({ sucursal_id: sucursal.id, servicio_id })) })
    ]);
...
export const setImagenes = async (req: Request, res: Response) => {
  try {
    const sucursal = (req as any).sucursal ?? (await prisma.sucursales.findUnique({ where: { id: Number(req.params.id) } }));
    if (!sucursal) return res.status(404).json({ error: 'Sucursal no encontrada' });
    ...
    const urls = Array.isArray(imagenes)
      ? [...new Set(imagenes.map((v: unknown) => String(v).trim()).filter((s: string) => s.length > 0))]
      : [];
    if (urls.length > 5) {
