...
export const list = async (req: Request, res: Response) => {
  try {
    ...
    if (activo !== undefined) where.activo = activo === '1' || activo === 'true';
...
export const create = async (req: Request, res: Response) => {
  try {
    ...
    if (cedula) {
      ...
      if (conCedula) return res.status(400).json({ error: 'La cédula ya está registrada' });
    }
    ...
    if (!rolPrincipal && rolesArr.length > 0) rolPrincipal = rolesArr[0];
    ...
    const finalRoles = new Set<number>();
    if (rolPrincipal) finalRoles.add(rolPrincipal);
...
export const update = async (req: Request, res: Response) => {
  try {
    ...
    if (req.body.clave) data.clave = await bcrypt.hash(req.body.clave, 10);
...
export const toggleActivo = async (req: Request, res: Response) => {
  try {
    ...
    if (id === req.user.id) return res.status(400).json({ error: 'No puedes bloquear tu propia cuenta' });
...
export const assignRoles = async (req: Request, res: Response) => {
  try {
    ...
    const roles: number[] = Array.isArray(req.body.roles)
      ? (req.body.roles as unknown[]).map((n) => Number(n)).filter((n) => Number.isInteger(n))
      : [];
    ...
    const ops: any[] = [prisma.usuario_roles.deleteMany({ where: { usuario_id: id } })];
    if (unique.length > 0) {
...
export const hacerDueno = async (req: Request, res: Response) => {
  try {
    const id = Number(req.params.id);
    ...
    const ops: any[] = [prisma.usuarios.update({ where: { id }, data: { rol_id: rolDueno.id } })];
    if (!yaTieneRol) {
