...
export const misEstadisticas = async (req: Request, res: Response) => {
  try {
    ...
    const finQuery = req.query.fin as string | undefined;
    ...
    const [
      totalVisitas, totalFavoritos, totalResenas, promedioCalificacion,
      reservasPorEstado, reservasVisitaPorEstado, habitacionesReservadas, cuponesCanjeados,
    ] = await Promise.all([
    ...
    // Desglose mensual
    const mensual: { mes: string; visitas: number; favoritos: number; resenas: number; reservas: number; habitaciones: number; cupones: number }[] = [];
    ...
    // Stats por sucursal
    const statsPorSucursal = await Promise.all(
    ...
    const negocios = await Promise.all(
      empresas.filter((e) => empresaIds.includes(e.id)).map(async (emp) => {
...
const misEstadisticasSucursal = async (
  _req: Request,
  res: Response,
  suc: { id: number; nombre: string; empresa_id: number },
  fechaInicio: Date,
  fechaFin: Date
) => {
  try {
