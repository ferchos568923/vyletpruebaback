...
import {
  suscripcionActivaPorCedula,
  DIAS_DEFAULT_SUSCRIPCION,
  limiteSucursalesPorCedula,
  limiteProductosPorCedula,
  permiteCuponesPorCedula,
  permiteReservasPorCedula,
  permiteCartillasPorCedula
...
export const obtenerMiPlan = async (req: Request, res: Response) => {
  try {
    const usuario = await prisma.usuarios.findUnique({
      where: { id: req.user.id },
      select: { cedula: true }
...
export const activarPlan = async (req: Request, res: Response) => {
  try {
    ...
    if (plan.activo === false) return res.status(400).json({ error: 'El plan no esta activo' });
...
export const solicitarMiPlan = async (req: Request, res: Response) => {
  try {
    const usuario = await prisma.usuarios.findUnique({
      where: { id: req.user.id },
      select: { cedula: true }
    ...
    const cedula = usuario?.cedula ?? '';
    if (!cedula) return res.status(400).json({ error: 'Tu usuario no tiene cedula registrada' });
