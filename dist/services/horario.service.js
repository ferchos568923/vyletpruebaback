...
const LARGOS = {};
for (const d of DIAS)
    LARGOS[normalizar(d.largo)] = d.id;
const CORTOS = {};
for (const d of DIAS)
    CORTOS[normalizar(d.corto)] = d.id;
const normalizarHora = (t) => {
    const limpio = (t ?? '').trim();
    if (!limpio)
        return '';
    if (limpio.includes(':')) {
...
export function estadoAbierto(horario, momento) {
