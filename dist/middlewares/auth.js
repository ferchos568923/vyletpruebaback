...
export const authenticateOpcional = async (req, res, next) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer '))
        return next();
...
export const esEmpleadoActivo = async (userId) => {
...
export const canGestionarSucursal = async (req, sucursal) => {
    if (isStaff(req))
        return true;
    ...
    if (negocio?.propietario === req.user.correo)
        return true;
    const emp = await prisma.empresa_empleados.findFirst({
...
export const empleadoTieneSucursal = async (userId, sucursalId) => {
    const emp = await prisma.empresa_empleados.findFirst({
...
export const sucursalesAsignadasEmpleado = async (userId, empresaId) => {
    const emp = await prisma.empresa_empleados.findFirst({
...
export const requireSucursalAcceso = (plataformaPermiso) => {
...
export const hasPermiso = (req, permiso) => !!req.permisos && req.permisos.includes(permiso);
...
export const canGestionarEmpresa = async (req, empresaId) => {
    if (isStaff(req))
        return true;
    const link = await prisma.usuario_empresas.findFirst({
...
export const canGestionarEmpleados = async (req, empresaId) => {
    if (isStaff(req))
        return true;
    const link = await prisma.usuario_empresas.findFirst({
        where: { usuario_id: req.user.id, empresa_id: empresaId }
...
export const canGestionarEmpresaConPermiso = async (req, empresaId, permisoNombre) => {
    if (isStaff(req))
        return true;
    const link = await prisma.usuario_empresas.findFirst({
        where: { usuario_id: req.user.id, empresa_id: empresaId }
    ...
    if (emp)
        return emp.empleado_permisos.some((p) => p.permisos_empresa.nombre === permisoNombre);
    const negocio = await prisma.empresas.findUnique({
        where: { id: empresaId },
        select: { propietario: true }
    });
