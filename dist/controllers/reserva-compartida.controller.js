...
export const compartirReserva = async (req, res) => {
    try {
        ...
        if (tipoReserva === 'mesa') {
            ...
            if (reserva)
                nombreLugar = reserva.sucursales?.nombre ?? '';
        }
            ...
            if (reserva)
                nombreLugar = reserva.habitaciones?.sucursales?.nombre ?? '';
        }
...
export const misReservasCompartidas = async (req, res) => {
    try {
        ...
        const resultado = await Promise.all(compartidas.map(async (c) => {
...
export const misReservasEnviadas = async (req, res) => {
    try {
        const compartidas = await prisma.reservas_compartidas.findMany({
...
export const responderCompartida = async (req, res) => {
    try {
        ...
        if (!compartida)
            return res.status(404).json({ error: 'Compartida no encontrada' });
