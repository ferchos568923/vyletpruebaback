...
export const getById = async (req, res) => {
    try {
        ...
        if (req.user) {
            ...
            const conGestion = await Promise.all((empresa.sucursales ?? []).map(async (s) => ({ ...s, puedoGestionar: await canGestionarSucursal(req, s) })));
...
export const create = async (req, res) => {
    try {
        const { nombre, categoria_id, descripcion, logo, email, sitio_web, facebook, instagram, tiktok, propietario, latitud, longitud } = req.body;
    ...
    try {
        ...
        if (isStaff(req)) {
            if (typeof req.body.verificado === 'boolean')
                data.verificado = req.body.verificado;
            if (typeof req.body.destacado === 'boolean')
                data.destacado = req.body.destacado;
            if (req.body.propietario !== undefined)
                data.propietario = req.body.propietario;
        }
