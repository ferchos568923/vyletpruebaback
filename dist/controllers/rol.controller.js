...
export const assignPermisos = async (req, res) => {
    try {
        const id = Number(req.params.id);
        const role = await prisma.roles.findUnique({ where: { id } });
        if (!role)
        ...
        const permisos = Array.isArray(req.body.permisos)
            ? req.body.permisos.map((n) => Number(n)).filter((n) => Number.isInteger(n))
            : [];
        const unique = [...new Set(permisos)];
        const ops = [prisma.rol_permisos.deleteMany({ where: { rol_id: id } })];
        if (unique.length > 0) {
