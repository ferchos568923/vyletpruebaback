...
const esHospedaje = async (sucursalId) => {
...
const validarImagenes = (imagenes) => {
    if (imagenes === undefined)
        return null; // no se envía → no se toca
    if (!Array.isArray(imagenes) || imagenes.length < 1 || imagenes.length > 5)
        return [];
...
const reemplazarImagenes = async (habitacionId, urls) => {
...
export const actualizarHabitacion = async (req, res) => {
    try {
        ...
        if (!(await esHospedaje(h.sucursales.id)))
            return res.status(404).json({ error: 'Habitaciones no disponibles para este negocio' });
        ...
        if (req.body.capacidad !== undefined)
            data.capacidad = req.body.capacidad === null ? null : Number(req.body.capacidad);
        if (req.body.precio !== undefined)
            data.precio = req.body.precio === null ? null : Number(req.body.precio);
        ...
        if (req.body.activa !== undefined)
            data.activa = req.body.activa === true;
...
export const disponibilidadSucursal = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        const { desde, hasta } = parseRango(req, 30);
        if (!(await esHospedaje(sucursalId)))
            return res.json({ desde, hasta, habitaciones: [] });
        const habitaciones = await prisma.habitaciones.findMany({
            where: { sucursal_id: sucursalId, activa: true },
            orderBy: { id: 'asc' },
            include: {
...
export const crearReservaHabitacion = async (req, res) => {
    try {
        ...
        const creadas = await prisma.$transaction(Array.from({ length: cantNum }).map(() => prisma.reservas_habitacion.create({
            data: {
                habitacion_id: habitacionId,
...
export const crearReservaHabitacionMulti = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        ...
        const ids = [...new Set(habitacion_ids.map(Number).filter((n) => Number.isInteger(n) && n > 0))];
        ...
        for (const h of habitaciones) {
            ...
            if (solapadas + nc > (h.cantidad ?? 1))
                ocupadas.push(h.nombre ?? `#${h.id}`);
        }
        if (ocupadas.length) {
            return res.status(409).json({ error: `Sin disponibilidad en esas fechas para: ${ocupadas.join(', ')}` });
        }
        const creadas = await prisma.$transaction(ids.flatMap((hid) => {
            const idx = ids.indexOf(hid);
            const repeticiones = Array.from({ length: cantPorHab[idx] });
            return repeticiones.map(() => prisma.reservas_habitacion.create({
                data: {
                    habitacion_id: hid,
                    usuario_id: req.user.id,
                    fecha_entrada: entrada,
...
export const crearReservaManual = async (req, res) => {
    try {
        ...
        if (!habitacion)
            return res.status(404).json({ error: 'Habitación no encontrada en esta sucursal' });
        const entrada = new Date(String(fecha_entrada));
        const salida = new Date(String(fecha_salida));
        if (isNaN(entrada.getTime()) || isNaN(salida.getTime()))
            return res.status(400).json({ error: 'Fechas inválidas' });
...
export const cambiarEstadoReservaHabitacion = async (req, res) => {
    try {
        const id = Number(req.params.id);
        ...
        if (!(await esHospedaje(r.habitaciones.sucursal_id)))
            return res.status(404).json({ error: 'Habitaciones no disponibles para este negocio' });
...
export const misReservasHabitacion = async (req, res) => {
    try {
        const reservas = await prisma.reservas_habitacion.findMany({
