...
const buildData = (body) => {
    ...
    if (body.precio !== undefined)
        data.precio = parseDecimal(body.precio);
    if (body.precio_oferta !== undefined)
        data.precio_oferta = body.precio_oferta === '' || body.precio_oferta === null ? null : parseDecimal(body.precio_oferta);
    if (body.stock !== undefined)
        data.stock = body.stock === '' || body.stock === null ? null : Number(body.stock);
    if (body.categoria_producto_id !== undefined)
        data.categoria_producto_id = body.categoria_producto_id === '' || body.categoria_producto_id === null ? null : Number(body.categoria_producto_id);
    if (body.destacado !== undefined)
        data.destacado = parseBool(body.destacado);
    if (body.disponible !== undefined)
        data.disponible = parseBool(body.disponible);
...
export const adminCreate = async (req, res) => {
    try {
        ...
        const etiquetas = Array.isArray(etiqueta_ids)
            ? [...new Set(etiqueta_ids)].map((e) => ({ etiqueta_id: Number(e) }))
            : [];
...
export const adminUpdate = async (req, res) => {
    try {
        ...
        if (Array.isArray(etiqueta_ids)) {
            const ids = [...new Set(etiqueta_ids)].map((e) => Number(e));
            etiquetaOps = {
                deleteMany: {},
