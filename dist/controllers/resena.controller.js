...
export const listarPorSucursal = async (req, res) => {
    try {
...
export const actualizarMia = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        const { calificacion, comentario } = req.body;
        const nota = Number(calificacion);
        ...
        if (!resena)
            return res.status(404).json({ error: 'No tienes una reseña en esta sucursal' });
...
async function recalcular(sucursalId) {
...
export const eliminarMia = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        const resena = await prisma.resenas.findFirst({
            where: { usuario_id: req.user.id, sucursal_id: sucursalId }
