...
export const verConfig = async (req, res) => {
    try {
        ...
        if (!config || !config.activo)
            return res.status(404).json({ error: 'Esta sucursal no tiene cartilla de fidelización' });
...
export const adminVer = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
...
export const adminGuardar = async (req, res) => {
    try {
        ...
        if (!sellos || sellos < 2 || sellos > 50)
            return res.status(400).json({ error: 'La meta debe ser entre 2 y 50 sellos' });
        if (!premio || !String(premio).trim())
            return res.status(400).json({ error: 'El premio es requerido' });
...
export const adminSellos = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
...
export const misCartillas = async (req, res) => {
    try {
...
export const unirse = async (req, res) => {
    try {
        ...
        const config = await prisma.cartilla_config.findUnique({ where: { sucursal_id: sucursalId } });
        if (!config || !config.activo)
            return res.status(404).json({ error: 'Esta sucursal no tiene cartilla activa' });
...
export const generarQR = async (req, res) => {
    try {
        ...
        if (cartilla.usuario_id !== req.user.id)
            return res.status(403).json({ error: 'No autorizado' });
        if (cartilla.estado !== 'activa')
            return res.status(400).json({ error: 'Cartilla ya completada' });
        if (!cartilla.cartilla_config.activo)
            return res.status(400).json({ error: 'Cartilla desactivada por el negocio' });
...
export const verificarQR = async (req, res) => {
    try {
        ...
        if (!payload)
            return res.status(400).json({ error: 'QR inválido o expirado', code: 'QR_EXPIRADO' });
        const cartilla = await prisma.cartilla_cliente.findUnique({
        ...
        if (!cartilla || cartilla.usuario_id !== payload.uid)
            return res.status(404).json({ error: 'Cartilla no encontrada' });
        if (cartilla.estado === 'canjeada')
            return res.status(400).json({ error: 'Premio ya entregado' });
        if (cartilla.estado !== 'activa')
            return res.status(400).json({ error: 'Cartilla ya completada' });
...
export const confirmarSello = async (req, res) => {
    try {
        const { cartilla_id } = req.body ?? {};
        const cartillaId = Number(cartilla_id);
        if (!cartillaId)
            return res.status(400).json({ error: 'cartilla_id requerido' });
        const cartilla = await prisma.cartilla_cliente.findUnique({
            where: { id: cartillaId },
            include: { cartilla_config: { include: { sucursales: { select: { id: true, nombre: true } } } } },
        });
...
export const adminCartillas = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        if (!(await verificarAccesoSucursal(req.user.id, sucursalId)))
            return res.status(403).json({ error: 'No tienes acceso a esta sucursal' });
        ...
        const where = { config_id: config.id };
        if (estado === 'activa' || estado === 'completada' || estado === 'canjeada')
...
export const entregarPremio = async (req, res) => {
    try {
        const cartillaId = Number(req.params.id);
        const cartilla = await prisma.cartilla_cliente.findUnique({
            where: { id: cartillaId },
        ...
        if (!cartilla)
            return res.status(404).json({ error: 'Cartilla no encontrada' });
        if (cartilla.estado !== 'completada')
            return res.status(400).json({ error: 'Solo se puede entregar el premio de una cartilla completada' });
        const sucursalId = cartilla.cartilla_config.sucursales.id;
        if (!(await verificarAccesoSucursal(req.user.id, sucursalId)))
            return res.status(403).json({ error: 'No tienes acceso a esta sucursal' });
