...
export const reservar = async (req, res) => {
    try {
        ...
        if (!cancha)
            return res.status(404).json({ error: 'Cancha no encontrada o inactiva' });
        const fechaDate = new Date(fecha);
        if (isNaN(fechaDate.getTime()))
            return res.status(400).json({ error: 'fecha inválida' });
        const horaInicio = timeToDate(hora_inicio);
        if (!horaInicio)
            return res.status(400).json({ error: 'hora_inicio inválida' });
...
export const crearManualReservaCancha = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        const sucursal = await prisma.sucursales.findUnique({
            where: { id: sucursalId },
...
export const listarSucursal = async (req, res) => {
    try {
        const sucursalId = Number(req.params.id);
        const sucursal = await prisma.sucursales.findUnique({
            where: { id: sucursalId },
...
export const editar = async (req, res) => {
    try {
        ...
        const data = {};
        if (cantidad_jugadores !== undefined)
            data.cantidad_jugadores = Number(cantidad_jugadores);
        if (observaciones !== undefined)
            data.observaciones = observaciones || null;
        if (cliente_nombre !== undefined)
            data.cliente_nombre = cliente_nombre || null;
        if (cliente_telefono !== undefined)
            data.cliente_telefono = cliente_telefono || null;
        if (cliente_correo !== undefined)
            data.cliente_correo = cliente_correo || null;
